            ...
            <div class="w-full max-w-5xl">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between mb-lg gap-4">
                    <div>
                        ...
                        <p class="text-body-md text-on-surface-variant">Create a new entry for your store's user.</p>
                    </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                                <div class="md:col-span-2 flex flex-col gap-xs">
                                    <form action="action.php?aksi=update" method="post" enctype="multipart/form-data">
                                        ...
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. Nike" type="text" name="nama_user" value="<?php echo $row['nama'] ?>" />
                                        <label class="font-label-md text-on-surface-variant">Alamat</label>
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. Rp 2000" type="text" name="alamat" value="<?php echo $row['alamat'] ?>" />
                                        <label class="font-label-md text-on-surface-variant">No_hp</label>
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. Nike" type="text" name="no_hp" value="<?php echo $row['no_hp'] ?>" />
                                        <label class="font-label-md text-on-surface-variant">Username</label>
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. Nike" type="text" name="username" value="<?php echo $row['username'] ?>" />
                                        <label class="font-label-md text-on-surface-variant mt-3">Password</label>
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. ********" type="password" name="password" value="<?= $row['password'] ?>" />
                                        <label class="font-label-md text-on-surface-variant">Role</label>
                                        <select class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" name="id_role">
                                            <?php
                                            $dataRole = showDataRole($conn);

                                            while ($roleItem = $dataRole->fetch_assoc()) {
                                                
                                            ?>

                                                <option value="<?= $roleItem['id'] ?>"
                                                    <?= ($row['id_role'] == $roleItem['id']) ? 'selected' : '' ?>>
                                                    <?= $roleItem['nama'] ?>
                                                </option>
