...
$id = $_GET['id'];
$data = showDataEditProduct($conn, $id);
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                                <div class="md:col-span-2 flex flex-col gap-xs">
                                    <form action="action.php?aksi=update" method="post" enctype="multipart/form-data">
                                        ...
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. Nike" type="text" name="nama_product" value="<?php echo $row['nama'] ?>" />
                                        <label class="font-label-md text-on-surface-variant">Price</label>
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. Rp 2000" type="text" name="price" value="<?php echo $row['harga'] ?>" />
                                        <label class="font-label-md text-on-surface-variant">Deskripsi</label>
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. Nike" type="text" name="deskripsi" value="<?php echo $row['deskripsi'] ?>" />
                                        <label class="font-label-md text-on-surface-variant">Image Product</label>
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. Nike" type="file" name="image_product" value="<?php echo $row['image'] ?>" />
                                        <label class="font-label-md text-on-surface-variant">Stock</label>
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. Nike" type="text" name="stock" value="<?php echo $row['stock'] ?>" />
                                        ...
                                        <select class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" name="id_category" id="">
                                            <?php
                                            $dataRole = showDataCategory($conn);

                                            while ($categoryItem = $dataRole->fetch_assoc()) {
                                                
                                            ?>

                                                <option value="<?= $categoryItem['id'] ?>"
                                                    <?= ($row['id_category'] == $categoryItem['id']) ? 'selected' : '' ?>>
                                                    <?= $categoryItem['nama'] ?>
                                                </option>
